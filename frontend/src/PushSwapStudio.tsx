...
function replayMoves(values: number[], moves: string[], cursor: number) {
    return moves
        .slice(0, cursor)
        .reduce((current, move) => applyMove(current, move), {
            a: values,
            b: [] as number[],
        });
}
...
export function PushSwapStudio({
    ...
    const [status, setStatus] = useState(
        "Paste values or generate a random set.",
    );
    ...
    async function runSort() {
        ...
        try {
            ...
            setCursor(0);
            setStatus(
                `${data.algorithm} algorithm returned ${data.move_count} moves.`,
            );
        } catch (error) {
            setStatus(
                error instanceof Error ? error.message : "push_swap failed.",
            );
        } finally {
            setIsBusy(false);
        }
            ...
            <div className="grid min-h-[720px] lg:grid-cols-[minmax(0,1fr)_380px]">
                ...
                >
                    ...
                    <label className="grid gap-2">
                        <span className="text-sm font-bold text-[#777267]">
                            Animation speed{" "}
                            <b className="text-[#30302e]">{speed} ms</b>
                        </span>
                        <input
                    ...
                    <p className="min-h-6 text-sm font-medium text-[#5e5d59]">
                        {status}
                    </p>
                    <section className="overflow-hidden rounded-xl border border-[#e8e3d6] bg-[#fffdf8]">
                        <button
                            ...
                            onClick={() =>
                                setIsMovesExpanded((current) => !current)
                            }
                            type="button"
                        >
    ...
    return (
        <section className="flex min-h-[420px] min-w-0 flex-col rounded-2xl border border-[#e8e3d6] bg-[#fffdf8]">
            <header className="flex items-center justify-between border-b border-[#ece8dc] px-4 py-3">
                ...
                <span className="font-bold text-[#777267]">
                    {values.length}
                </span>
            </header>
